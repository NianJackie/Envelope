...
class EnveController extends InterceptController {
 ...
 /*   	$adv = AdvController::instance()->getAdv('hbsquare');exit(json_encode($adv));exit();
    ...
    public function saveEnveReceive(){
        ...
        if($count >  0){
            $this->ajaxReturn(['code'=>40500, 'msg'=>'已领此取过红包']);
        }

        //检测本赛季是否领取过红包
        $enve_receive_model = D("EnveReceive");
        $count = $enve_receive_model->where("game_id='%d' and user_id='%s'",array($info['game_id'], $this->user_id))->count();
        if($count >  0){
            $this->ajaxReturn(['code'=>40500, 'msg'=>'本赛季已领取过红包']);
        }
        ...
        if ($info['enve_type'] != 1) {
            ...
            $post_data['voice_url'] = $home.'/data/upload/default/20180204/5a76ee153a3f3.silk';
            //获取识别结果
